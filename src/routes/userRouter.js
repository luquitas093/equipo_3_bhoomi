...
const { route } = require("./adminRouter.js");
...
const registerValidation = [ 
  body('first_name')
   ...
   body('phone')
                .notEmpty().withMessage('El campo télefono es obligatorio').bail()
                .isNumeric().withMessage('Por favor, ingrese sólo números').bail()
                .isLength({min: 10, max: 13 }) ('Ingrese el prefijo sin el 0 más su número teléfonico'),
   ...
   body('email')
                ...
                .isEmail().withMessage('Por favor, ingrese un E-Mail válido'),
   body('password').isLength({min: 8 }).withMessage('La contraseña tiene que tener un mínimo de 8 caracteres'),
